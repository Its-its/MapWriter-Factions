...
public abstract class MwGuiSlot {
	...
	public int getSlotIndexFromScreenCoords(int x, int y) {
		...
		for (int index = 0; index < this.getSize(); ++index) {
			...
			//If: Quick Fix so it doesn't crash when right clicking no slot.
	...
	public boolean isMouseInField() {
		return this.isMouseYWithinSlotBounds()
				&& this.isMouseXWithinSlotBounds();
	}
	...
	protected void drawSelectionBox(int x, int y, int mouseXIn, int mouseYIn) {
		...
		for (int index = 0; index < i1; ++index) {
			...
			if (this.showSelectionBox && this.isSelected(index)) {
				...
				int xRight = (this.left + ((this.width / 2) + (this
						.getListWidth() / 2))) - this.scrollBarWidth;

				GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
				GlStateManager.disableTexture2D();
				worldrenderer.startDrawingQuads();
				worldrenderer.setColorOpaque_I(8421504);
				worldrenderer.addVertexWithUV(xLeft, yTotal + slotHeight, 0.0D,
						0.0D, 1.0D);
				worldrenderer.addVertexWithUV(xRight, yTotal + slotHeight,
						0.0D, 1.0D, 1.0D);
				worldrenderer.addVertexWithUV(xRight, (yTotal), 0.0D, 1.0D,
						0.0D);
				worldrenderer
						.addVertexWithUV(xLeft, (yTotal), 0.0D, 0.0D, 0.0D);
				worldrenderer.setColorOpaque_I(0);
				worldrenderer.addVertexWithUV(xLeft + 1,
						(yTotal + slotHeight) - 1, 0.0D, 0.0D, 1.0D);
				worldrenderer.addVertexWithUV(xRight - 1,
						(yTotal + slotHeight) - 1, 0.0D, 1.0D, 1.0D);
				worldrenderer.addVertexWithUV(xRight - 1, yTotal + 1, 0.0D,
						1.0D, 0.0D);
				worldrenderer.addVertexWithUV(xLeft + 1, yTotal + 1, 0.0D,
						0.0D, 0.0D);
				tessellator.draw();
				GlStateManager.enableTexture2D();
			}
