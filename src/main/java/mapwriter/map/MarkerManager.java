...
public class MarkerManager {
	...
	public void load(Configuration config, String category) {
		...
		if (config.hasCategory(category)) {
			...
			this.visibleGroupName = config.get(category, "visibleGroup", "")
					.getString();
			...
			if (markerCount > 0) {
				for (int i = 0; i < markerCount; i++) {
					...
					if (marker != null) {
						...
						Logging.log("error: could not load " + key
								+ " from config file");
					}
				}
			}
	...
	public String markerToString(Marker marker) {
		return String.format("%s:%d:%d:%d:%d:%06x:%s", marker.name, marker.x,
				marker.y, marker.z, marker.dimension, marker.colour & 0xffffff,
				marker.groupName);
	}
	...
	public Marker stringToMarker(String s) {
		...
		if (split.length == 7) {
			try {
				...
				marker = new Marker(split[0], split[6], x, y, z, dimension,
						colour);
	...
	public void addMarker(String name, String groupName, int x, int y, int z,
			int dimension, int colour) {
		this.addMarker(new Marker(name, groupName, x, y, z, dimension, colour));
		this.save(WorldConfig.getInstance().worldConfiguration,
	...
	public void update() {
		...
		for (Marker marker : this.markerList) {
			if (marker.groupName.equals(this.visibleGroupName)
					|| this.visibleGroupName.equals("all")) {
	...
	public Marker getNearestMarkerInDirection(int x, int z, double desiredAngle) {
		...
		for (Marker marker : this.visibleMarkerList) {
			...
			if ((Math.cos(desiredAngle - angle) > 0.8D)
					&& (d < nearestDistance) && (d > 4)) {
				nearestMarker = marker;
				nearestDistance = d;
			}
		}
	...
	public void drawBeam(Marker m, float partialTicks) {
		...
		float f2 = Minecraft.getMinecraft().theWorld.getTotalWorldTime()
				+ partialTicks;
		...
		worldrenderer.pos(x + d5, y + d17, z + d6)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(x + d5, y, z + d6).color(fRed, fGreen, fBlue, fAlpha)
				.endVertex();
		worldrenderer.pos(x + d7, y, z + d8).color(fRed, fGreen, fBlue, fAlpha)
				.endVertex();
		worldrenderer.pos(x + d7, y + d17, z + d8)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(x + d11, y + d17, z + d12)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(x + d11, y, z + d12)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(x + d9, y, z + d10)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(x + d9, y + d17, z + d10)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(x + d7, y + d17, z + d8)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(x + d7, y, z + d8).color(fRed, fGreen, fBlue, fAlpha)
	...
	public void drawLabel(Marker m) {
		...
		worldrenderer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_COLOR);
		worldrenderer.pos(-strTextWidth - 1, (-1), 0.0D)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(-strTextWidth - 1, (8), 0.0D)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(strTextWidth + 1, (8), 0.0D)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(strTextWidth + 1, (-1), 0.0D)
		...
		worldrenderer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_COLOR);
		worldrenderer.pos(-strDistanceWidth - 1, -1 + offstet, 0.0D)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(-strDistanceWidth - 1, 8 + offstet, 0.0D)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(strDistanceWidth + 1, 8 + offstet, 0.0D)
				.color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(strDistanceWidth + 1, -1 + offstet, 0.0D)
